...
If you want to use randomizer with other file-based mods, you will be able to specify another mod directory in the randomizer program using "Merge other mod". In order to merge a regulation.bin file, select the directory it lives in. When randomization is applied, it will automatically merge this mod. When it launches, it will launch using the merged files plus the mod's other files.
...
If you have other file-based mods to merge with randomizer, use "Merge other mod". You can select a mod directory to merge, or enter a mod directory name manually. When the randomizer writes its files, it will use files from that directory as a base, including the regulation.bin file. (Also with Manual Mod Engine, you need to edit the config file [like this](https://cdn.discordapp.com/attachments/936393685585780846/980279218602643536/unknown.png), as mentioned in Step 2b). Be warned that some mods cannot be merged with item randomizer currently, and will always result in errors.
